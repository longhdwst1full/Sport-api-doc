...
async function seed(transaction: Prisma.TransactionClient): Promise<void> {
  ...
  const existingBootstrapUser = await transaction.user.findFirst({
  ...
  const bootstrapUser = existingBootstrapUser
    ...
    }).then(() => transaction.user.findFirstOrThrow({
      where: { normalizedEmail: BOOTSTRAP_ADMIN.EMAIL },
    }))
