...
1. Define the user journey, SEO/indexing need, authentication boundary and offline contract A/B/C.
2. Choose server rendering by default for public read content; isolate interactive client components narrowly.
3. Use only public generated operations and map DTOs to display models inside the owning feature when needed.
4. Implement loading, empty, not-found, error and offline behavior, including stale price/stock handling.
5. Make variant/combo selection and purchase constraints explicit before add-to-cart.
