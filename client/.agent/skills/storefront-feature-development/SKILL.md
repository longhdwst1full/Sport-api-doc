...
2. Choose server rendering by default for public read content; follow `server-first route -> feature composition -> narrow client hook/island -> public generated SDK` and isolate interactive client components narrowly.
3. Compose pages through the current `layouts`/`widgets`/`features` boundaries. Use only public generated operations and map DTOs inside the owning feature. Do not copy the reference Vite apps' global Redux flow into public Next.js routes.
