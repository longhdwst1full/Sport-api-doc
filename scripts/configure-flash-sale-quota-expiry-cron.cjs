...
async function set(prisma) {
  ...
  const schedule = process.env.FLASH_SALE_QUOTA_EXPIRY_CRON_SCHEDULE?.trim() || '1-59/5 * * * *'; // Lech phut voi reservation-expiry (*/5) de khong ban cung giay: 4 job cung luc tung lam pg_net ket DNS/timeout 30s.
