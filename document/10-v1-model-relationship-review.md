...
## Quy ước ID và tương thích dữ liệu cũ

- Mọi entity mới dùng `BIGINT IDENTITY`; PostgreSQL là nơi duy nhất cấp ID.
- API nhận/trả ID dạng chuỗi số. NestJS chuyển chuỗi sang `bigint` tại controller/service boundary và không serialize `bigint` trực tiếp.
- 21 bảng vật lý đã tồn tại trước D43 có `legacy_id UUID UNIQUE NULL`: dữ liệu cũ được giữ nguyên UUID tại đây; dữ liệu mới để `NULL`.
- Tất cả FK thật được remap sang khóa số trong một transaction. `audit_logs.entity_id` và `inventory_movements.reference_id` chuyển thành `TEXT`, cho phép truy vết cả UUID lịch sử và ID số mới vì hai cột này là liên kết đa hình, không phải FK.
- Seed/upsert tra theo business key (`code`, `normalized_email`, `sku`, `product_no`), không cố định ID. Điều này giữ seed lặp lại an toàn sau khi DB tự cấp ID.
- Migration D43 dùng advisory lock và `ACCESS EXCLUSIVE`; lần deploy đầu cần backup và maintenance window. Với nhiều replica, ưu tiên một release job `APP_MODE=migrate`; các replica ứng dụng vẫn fail-fast nếu schema chưa sẵn sàng.
