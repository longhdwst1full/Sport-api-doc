...
- [x] Prisma schema: 21 bảng vật lý dùng BIGINT IDENTITY; FK dùng BIGINT; `legacy_id UUID UNIQUE NULL` bảo toàn UUID cũ.
- [x] NestJS: validator/pipe ID số tập trung; API serialize ID thành chuỗi; UUID chỉ còn cho request/idempotency correlation.
- [x] Seed: upsert theo business key, không gán ID cố định, không ghi đè password Admin đã đổi.
- [x] Migration versioned: `20260905120000_migrate_uuid_ids_to_bigint_identity`, transaction + advisory/table lock + FK remap + constraint/index rebuild.
...
| Unit + integration + HTTP tests | 20 | **18** | Unit và compile gate xanh; integration/HTTP evidence D43 phải chạy lại sau khi apply migration thứ 10 |
| Migration + seed + audit + contract docs | 10 | **8** | Migration/schema/OpenAPI/DBML/catalog/change-log/workbook đã cập nhật; chưa apply/verify migration D43 trên Supabase |
...
- [x] Gate D43 ngày 2026-09-05: API lint, Prisma validate/generate, Nest build và **29 suites/104 tests**; Admin lint, **12 files/22 tests**, Vite build và Storybook build; Client typecheck, **3 files/4 tests** và Next.js build đều pass.
...
| --- | --- | --- | --- |
| 1.9.1 | 2026-09-05 | Đồng bộ DoD 90% và ghi evidence gate D43 mới nhất. | D43 verification |
| 1.9.0 | 2026-09-05 | Chuyển ID sang BIGINT IDENTITY, migrate-at-start và regenerate contract/SDK. | D43 |
